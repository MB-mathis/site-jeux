...
$itemsParPage = 2; // Afficher LE NOMBRE DE jeux par page
...
<div class="jeux-container">
    <?php foreach ($jeux as $jeu): ?>
        <div class="jeu">
            <a href="details-jeu.php?id=<?= $jeu['id_jeux'] ?>">
                <img src="<?= htmlspecialchars($jeu['image']) ?>" alt="<?= htmlspecialchars($jeu['nom']) ?>" class="jeu-image">
                <div class="jeu-info">
...
<?php if ($totalPages > 1): ?>
    ...
    <!-- Première -->
    <a href="?page=1" <?= ($pageActuelle == 1) ? 'class="active"' : '' ?>>Première</a>

    <!-- Précédente -->
    <a href="?page=<?= max(1, $pageActuelle - 1) ?>" <?= ($pageActuelle == 1) ? 'class="disabled"' : '' ?>>Précédente</a>
    ...
    <!-- Suivante -->
    <a href="?page=<?= min($totalPages, $pageActuelle + 1) ?>" <?= ($pageActuelle == $totalPages) ? 'class="disabled"' : '' ?>>Suivante</a>

    <!-- Dernière -->
    <a href="?page=<?= $totalPages ?>" <?= ($pageActuelle == $totalPages) ? 'class="active"' : '' ?>>Dernière</a>
